...
//   string outputFile     = ""
  ...
  samples.push_back("ttbar_lf.root");
...
//   catList.push_back("ge5je3t");
  ...
  for( unsigned int i = 0 ; i < nsamples ; ++i ){
    ...
    string inputFile      = "inputTrees/" + samples[i] ;
...
//     Float_t                 varHt;
	...
	//reader[j]->AddVariable( "min_dr_tagged_jets",         &varmin_dr_tagged_jets   ); 
      ...
      TString dir    = "weights/" + label + "/";
      TString prefix = "TMVAClassification";
      
      // Book method(s)
      if(Use["BDTG"]) reader[j]->BookMVA( "BDTG method", dir + prefix + TString("_BDTG.weights.xml"));  
      
      //Add new branches for MVA output
      branchBDTG[j] = 0 ; 
      TString branchName = "BDTG_CHARLIE" + label;
      TString branchType = branchName + "/F";
      if(Use["BDTG"])  branchBDTG[j] = OutputTree->Branch( branchName, &fBDTG[j], branchType );
    ...
    Float_t sum_pt;
    Float_t min_dr_jets;
    Float_t higgsLike_dijet_mass;
    Float_t higgsLike_dijet_mass2;
    Float_t numHiggsLike_dijet_15_float;
...
//     Float_t numTag;
    ...
    inTree->SetBranchAddress( "sum_pt", &sum_pt);
    inTree->SetBranchAddress( "min_dr_jets", &min_dr_jets) ;
    inTree->SetBranchAddress( "higgsLike_dijet_mass", &higgsLike_dijet_mass);
    inTree->SetBranchAddress( "higgsLike_dijet_mass2", &higgsLike_dijet_mass2);    
    inTree->SetBranchAddress( "avg_btag_disc_btags", &avg_btag_disc_btags);
    inTree->SetBranchAddress( "numHiggsLike_dijet_15_float", &numHiggsLike_dijet_15_float);
...
//     inTree->SetBranchAddress( "numTaggedJets_float", &numTag);
    ...
    for (Long64_t ievt=0; ievt<inTree->GetEntries();ievt++) {
      ...
      varhiggsLike_dijet_mass = higgsLike_dijet_mass;
      varhiggsLike_dijet_mass2 = higgsLike_dijet_mass2;
      varavg_btag_disc_btags = avg_btag_disc_btags;      
      varnumHiggsLike_dijet_15_float = numHiggsLike_dijet_15_float;
...
//       varHt                              = Ht;    
	...
	if(Use["BDTG"]) { 
	  fBDTG[j]            = tmpReader->EvaluateMVA( "BDTG method" );
	  branchBDTG[j]->Fill();
	}
      } // End category loop
